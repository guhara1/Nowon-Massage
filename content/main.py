...
<p>노원구에서 방문 마사지와 홈타이 예약을 찾는 분들을 위해 가능 지역, 예약 절차, 코스 선택 기준, 이용 전 확인사항을 한곳에 정리했습니다. 이 페이지는 노원구 전체 구조를 설명하는 허브 역할을 하며, 더 자세한 내용은 지역별·지하철역별·테마별 안내 페이지에서 확인하실 수 있습니다. {BRAND}는 예약 확인부터 방문 관리까지 정해진 절차에 따라 진행하며, 처음 이용하시는 분도 어렵지 않게 예약할 수 있도록 각 단계를 명확하게 안내해 드립니다. 자택, 오피스텔, 숙소 등 조용한 공간이 확보되는 곳이라면 어디든 방문 상담이 가능합니다.</p>
</section>

<section id="coverage">
<h2>노원구 전지역 방문 가능 안내</h2>
<p>노원구 지역 안내는 월계동, 공릉동, 하계동, 중계동, 상계동 다섯 개 대표 동을 중심으로 구성되어 있습니다. 월계1동부터 월계3동, 상계1동부터 상계10동처럼 숫자로 나뉜 행정동은 별도 페이지를 만들지 않고 각 대표 동 페이지에서 통합하여 안내합니다. 같은 생활권을 잘게 쪼개 비슷한 내용을 반복하는 것보다, 동 단위로 묶어 생활권 특징과 방문 조건을 한 번에 설명하는 편이 이용자에게도 정확하기 때문입니다. 거주 중인 행정동이 어디든 대표 동 페이지에서 필요한 정보를 모두 확인하실 수 있습니다.</p>
</section>
...
<section id="course">
<h2>코스 선택 안내</h2>
<p>코스는 이용 목적과 그날의 컨디션에 따라 선택하시는 것이 좋습니다. 누적된 피로를 풀고 싶은 분, 편안한 휴식이 필요한 분, 운동 후 근육 이완이 필요한 분, 숙소로 방문을 원하시는 분, 커플이 함께 받고 싶은 분 등 상황에 맞는 선택 기준을 <a href="/courses/">코스안내</a> 페이지에서 자세히 다룹니다. 어떤 코스가 맞을지 고민되시면 예약 전화에서 상태를 말씀해 주세요. 맞는 방향을 함께 정해 드립니다.</p>
</section>
...
PAGE = {
    ...
    "desc": "노원 출장마사지·홈타이 안내 페이지입니다. 월계동, 공릉동, 하계동, 중계동, 상계동과 노원구 주요 지하철역 인근, 테마별 관리, 예약 전 확인사항을 확인해보세요.",
